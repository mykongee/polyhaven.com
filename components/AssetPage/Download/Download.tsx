...
const Download = ({ assetID, data, files, setPreview, patron }) => {
    ...
    useEffect(() => {
        setRes(
            localStorage.getItem(`assetPref_${data.type}_resolution`) || '4k'
        );
            ...
            <div className={styles.downloadBtnWrapper}>
                <div className={styles.downloadBtn}>
                    <span className={styles.error}>
                        ...
                        <br />
                        <em>
                            Try refresh, otherwise please report this to us.
                        </em>
                    </span>
                </div>
            </div>
    ...
    const dlRes = resOptionKeys.includes(prefRes)
        ? prefRes
        : resOptionKeys.slice(-1)[0];
    ...
    const fmtOptions = isHDRI
        ...
        : {
              ...
              gltf: {
                  label: 'glTF',
                  tooltip: `glTF 2.0, ${t('formats.some-software')}<br/>${t(
                      'formats.incl-tex'
                  )}`,
                  icon: <IconGltf />,
              },
    ...
    const dlFmt = Object.keys(fmtOptions).includes(prefFmt)
        ? prefFmt
        : isHDRI
        ? 'hdr'
        : 'blend';
    ...
    if (dlFmt !== 'zip') {
        ...
        for (const f of zipList.files) {
            ...
            if (f.map === 'gltf') {
                fsize +=
                    files[f.map][dlRes][f.fmt].include[`${assetID}.bin`].size;
            }
        }
    }
    ...
    const downloadZip = async () => {
        ...
        if (dlFmt !== 'zip') {
            ...
            for (const f of zipList.files) {
                const basePath = threeDFormats.includes(f.fmt)
                    ? ''
                    : 'textures/';
                const fileInfo = files[f.map][dlRes][f.fmt];
                dlFiles.push({
            ...
            <div className={styles.downloadBtnWrapper}>
                ...
                <a
                    ...
                    className={`${styles.downloadBtn} ${
                        busyDownloading ? styles.disabled : null
                    }`}
                ...
                >
                    ...
                    <div
                        className={`${styles.busyDownloading} ${
                            busyDownloading ? styles.show : null
                        }`}
                    >
                        <Loader />
                    </div>
