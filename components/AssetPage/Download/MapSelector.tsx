...
const MapSelector = ({ name, res, fmt, type, data, filesize, selectMap }) => {
    ...
    enabledByDefault.forEach(
        (name, index) =>
            (enabledByDefault[index] = `assetPref_sel_${type}_${name}`)
    );
    ...
    useEffect(() => {
        ...
        const v =
            storedValue !== null
                ? storedValue
                : enabledByDefault.includes(storageKey);
        setChecked(v);
