...
const Carousel = ({
    ...
    if (data) {
        for (const i of Object.keys(data)) {
            images[
                i
            ] = `https://cdn.polyhaven.com/asset_img/renders/${slug}/${i}`;
        ...
        <div className={styles.imageRow}>
            ...
            <div
                className={`${styles.iconBtn} ${
                    active === 'webGL' ? styles.activeImage : ''
                }`}
            >
            ...
            {renders.map((i, k) => (
                <div
                    ...
                    className={`${styles.image} ${
                        active === images[i] ? styles.activeImage : ''
                    }`}
                >
                        ...
                        <div className={styles.credit}>
                            ...
                            {image_info[i].sources ? (
                                <div className={styles.sources}>
                                    {image_info[i].sources.length > 1
                                        ? 'Sources:'
                                        : 'Source:'}
                                    <ul>
                                        {image_info[i].sources.map(
                                            (link, k) => (
                                                <li key={k}>
                                                    <a
                                                        href={link}
