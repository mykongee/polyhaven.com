...
const AssetPage = ({ assetID, data, files, renders }) => {
    ...
    const [activeImageSrc, setActiveImageSrc] = useState(
        `https://cdn.polyhaven.com/asset_img/primary/${assetID}.png`
    ); // Without height=X, used to highlight active image in carousel
    const [showWebGL, setShowWebGL] = useState(false);
    const [showTilePreview, setShowTilePreview] = useState('');
    const [showDownloadGraph, setShowDownloadGraph] = useStoredState(
        'asset_showDownloadGraph',
        false
    );
    ...
    const monthAgo = new Date(Date.now() - 30 * msPerDay)
        .toISOString()
        .split('T')[0];
    const daysOld = (Date.now() - data.date_published * 1000) / msPerDay;
    const isOlderThanFourDays =
        Date.now() - data.date_published * 1000 > 4 * msPerDay;
    ...
    useEffect(() => {
        ...
        if (!path) {
            path = `<a href="/${Object.keys(asset_types)[data.type]}">${tc(
                asset_type_names[data.type] + 's'
            )}</a> /`;
        }
        ...
        <div className={styles.wrapper}>
            <div
                className={`${styles.loading} ${
                    !pageLoading ? styles.hidden : null
                }`}
            >
                <Spinner />
            </div>
            <Page immersiveScroll assetPage>
                <div className={styles.mobileHeader}>
                    ...
                    <div
                        className={`${styles.sidebarToggle} ${
                            !hideSidebar ? styles.sidebarToggleClose : ''
                        }`}
                    >
                        <IconButton
                            icon={
                ...
                <div className={styles.previewWrapper}>
                    <div
                        className={`${styles.activePreview}${
                            showWebGL ? ' ' + styles.activePreviewGLTF : ''
                        }`}
                    >
                        <img
                            id="activePreview"
                        ...
                        <div
                            className={`${styles.loading} ${
                                !imageLoading ? styles.hidden : null
                            }`}
                        >
                            <Spinner />
                        </div>
            ...
            <div
                className={`${styles.sidebar} ${
                    hideSidebar ? styles.hiddenMobile : ''
                }`}
            >
                <div className={styles.info}>
                    ...
                    <div className={styles.infoItems}>
                        <InfoItem
                            label={t('author', { count: authors.length })}
                            flex
                        >
                        ...
                        <div className={styles.infoBlocks}>
                            ...
                            <InfoBlock
                                ...
                                label={ageLabel}
                                tip={new Date(
                                    data.date_published * 1000
                                ).toLocaleString('en-ZA')}
                            />
                            {data.dimensions ? (
                                <InfoBlock
                                    value={`${parseFloat(
                                        (data.dimensions[0] / 1000).toFixed(1)
                                    )}m`}
                                    label={t('wide')}
                        ...
                        <InfoItem
                            label={t('downloads')}
                            condition={Boolean(data.download_count)}
                            flex
                        >
                            <span
                                data-tip={`${Math.round(
                                    data.download_count / daysOld
                                )} ${t('downloads-pd')}`}
                            >
                                {data.download_count}
                            </span>
                        ...
                        <div ref={widthRef}>
                            <TagsList
                                ...
                                linkPrefix={`/${
                                    Object.keys(asset_types)[data.type]
                                }/`}
                                width={sidebarWidth}
                            />
                            <TagsList
                ...
                <div className={styles.sidebarAd}>
                    <DisplayAd id="9249051205" x={336} y={280} showRemoveBtn />
                </div>
            </div>
        </div>
    );
