...
const GLTFViewer: FC<Props> = ({ show, assetID, files, onLoad }) => {
    ...
    const [soloMap, setSoloMap] = useState<
        'NORMAL' | 'METALNESS' | 'ROUGHNESS' | 'DIFFUSE' | ''
    >('');
    ...
    const [state, dispatch] = useReducer(
        GLTF_Visibility_reducer,
        GLTF_Visibility_reducer_defaultState
    );
    ...
    const center = useMemo(
        () => calcSceneCenter(calcSceneBB(state.meshes)),
        [state.meshes]
    );
    const camDistance = useMemo(
        () => Math.ceil(state.boundingSphereRadius * 2.5),
                ...
                >
                    <Canvas
                        camera={{
                            ...
                            far: 1000,
                            position: [
                                -(center.x + camDistance),
                                center.y + camDistance / 2,
                                center.z + camDistance,
                            ],
                        }}
                        onCreated={({ gl }) => {
                        ...
                        <Suspense fallback={null}>
                            ...
                            {processedGLTFFromAPI &&
                                ...
                                Object.values(state.meshes).map((node) => {
                                    return (
                                        <React.Fragment key={node.mesh.uuid}>
                                            <mesh
                                                geometry={node.mesh.geometry}
                                                position={node.mesh.getWorldPosition(
                                                    new Vector3()
                                                )}
                                                scale={node.mesh.getWorldScale(
                                                    new Vector3()
                                                )}
                                                quaternion={node.mesh.getWorldQuaternion(
                                                    new Quaternion()
                                                )}
                                            >
                ...
                <div className={styles.buttons}>
                    ...
                    <IconButton icon={<MdLayers />}>
                        <IconButton
                            icon={
                                <img
                                    src={`https://cdn.polyhaven.com/site_images/map_types/DIFFUSE.png?width=32`}
                                />
                            }
                        ...
                        <IconButton
                            icon={
                                <img
                                    src={`https://cdn.polyhaven.com/site_images/map_types/NORMAL.png?width=32`}
                                />
                            }
                        ...
                        <IconButton
                            icon={
                                <img
                                    src={`https://cdn.polyhaven.com/site_images/map_types/METALNESS.png?width=32`}
                                />
                            }
                        ...
                        <IconButton
                            icon={
                                <img
                                    src={`https://cdn.polyhaven.com/asset_img/thumbs/rough_wood.png?width=32`}
                                />
                            }
                            active={soloMap === ''}
                            onClick={() => {
...
const calcSceneBB = (meshes): BoundingBox => {
    return Object.values(meshes).reduce<BoundingBox>(
        (acc, curr: MeshState, idx) => {
            curr.mesh.updateMatrixWorld();
            const box = new Box3();
            const wBounds = box
                .copy(curr.mesh.geometry.boundingBox)
                .applyMatrix4(curr.mesh.matrixWorld);
            if (idx === 0) {
                acc.min = wBounds.min;
                acc.max = wBounds.max;
            } else {
