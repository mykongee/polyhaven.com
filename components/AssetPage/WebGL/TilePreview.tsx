...
const TilePreview = ({ image, resolutions, type }) => {
    ...
    let displayedResolution = resList.reduce((prev, curr) =>
        Math.abs(curr - scale) < Math.abs(prev - scale) ? curr : prev
    );
    const prevResolution =
        resList[Math.max(0, resList.indexOf(displayedResolution) - 1)];
    const bgSize = startRes * zoom;
    const bgPosition = `${width / 2 + offset[0] * zoom}px ${
        height / 2 + offset[1] * zoom
    }px`;
    ...
    const startDrag = (e) => {
        if (e.button === 0) {
            // Left click
            setOffsetStart([
                e.pageX - offset[0] * zoom,
                e.pageY - offset[1] * zoom,
            ]);
            setDrag(true);
        }
        if (e.button === 2) {
    ...
    const mouseMove = (e) => {
        if (drag) {
            setOffset([
                (e.pageX - offsetStart[0]) / zoom,
                (e.pageY - offsetStart[1]) / zoom,
            ]);
        }
        if (dragZoom) {
            setOffsetStart([e.pageX, e.pageY]);
    ...
    return (
        <div className={styles.wrapper} ref={wrapperRef}>
            ...
            <div
                ...
                style={{
                    backgroundImage: `url(${imageAtRes(
                        image,
                        prevResolution
                    )})`,
            ...
            <div className={styles.buttons}>
                ...
                {grid && zoom > 1.5 ? (
                    <p>
                        You may need to zoom out to
                        <br />
                        see the tiling preview grid
                    </p>
                ) : null}
                {type === 1 ? (
                    <IconButton
                        icon={<MdGridOn />}
                        onClick={(_) => setGrid(!grid)}
                        active={grid}
                    />
                ) : null}
                <IconButton
                    icon={<MdRemove />}
                    onClick={(_) => setZoom(zoom / zoomFactor)}
                />
                <IconButton
                    icon={<MdAdd />}
                    onClick={(_) => setZoom(zoom * zoomFactor)}
                ...
                <IconButton
                    icon={<p>1:1</p>}
                    onClick={(_) => setZoom(1 / (startRes / 1024))}
                />
            </div>
        </div>
    );
