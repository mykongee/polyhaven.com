...
const AuthorCredit = ({ id, size, credit }) => {
    ...
    const { data, error } = apiSWR(`/author/${id}`, {
    ...
    if (donate && donate.startsWith('paypal:')) {
        ...
        donate = paypalEmailToLink(
            donate.slice('paypal:'.length),
            'Poly Haven: ' + name
        );
    }
    ...
    return (
        <div className={styles.author}>
            <Link href={assetLink} prefetch={false}>
                <a className={styles.avatar}>
                    ...
                    {data && data.regular_donor ? (
                        <div
                            className={styles.regularDonor}
                            data-tip="Regular asset donor"
                        >
                            <Heart color="#F96854" />
                        </div>
                    ) : null}
            ...
            <div className={styles.name}>
                ...
                {credit ? (
                    <span className={styles.credit}>{creditStr}</span>
                ) : (
                    ''
                )}
