...
const Bar = ({
    ...
    return (
        <div className={styles.barWrapper}>
                        ...
                        return (
                            <div
                                ...
                                style={{
                                    width: `${percent}%`,
                                    background:
                                        !filter.length || filter.includes(c)
                                            ? catColor(c)
                                            : 'rgba(0,0,0,0.1)',
                                }}
                            >
            ...
            <MdExpand
                className={styles.barListExpand}
...
const Monthly = ({
    ...
    for (const m of Object.keys(mutableData).slice(-12)) {
        for (const mode of ['income', 'expense']) {
            for (const [k, v] of Object.entries(mutableData[m][mode])) {
                ...
                const adjustedValue =
                    v /
                    (mutableData[m].rates[currency] /
                        mutableData[latestMonth].rates[currency]);
    ...
    const totalIncome: any = Object.values(mutableData[month].income).reduce(
        (a: number, b: number) => a + b,
        0
    );
    const totalExpense: any = Object.values(mutableData[month].expense).reduce(
        (a: number, b: number) => a + b,
        0
    );
        ...
        <div className={styles.barSection}>
            ...
            <ul>
                {currency !== 'ZAR' ? (
                    <li>
                        Exchange Rate in{' '}
                        {month === 'This Year' ? latestMonth : month}:{' '}
                        {getCurrency(1 * rates[currency], currency, rates)} ={' '}
                        {getCurrency(1 * rates[currency], 'ZAR', rates)}
                    </li>
                ) : null}
                <li>
                    Current Balance:{' '}
                    {getCurrency(balance, currency, latestRates)}
                    <MdHelp data-tip="Actual balance of our combined accounts, ignoring interest earned." />
                </li>
                <li>
                    Target Emergency Savings:{' '}
                    {getCurrency(targetEmergencyFund, currency, latestRates)}
                    <MdHelp
                ...
                {savings >= 0 ? (
                    <li>
                        <strong>
                            Usable Funds:{' '}
                            {getCurrency(savings, currency, latestRates)}
                        </strong>
