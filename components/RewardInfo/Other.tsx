...
const Other = ({ uuid, patron }) => {
    ...
    const doUpdateName = (_) => {
        setPatronInfo(uuid, { display_name: name.trim() || null }).then(
            (resdata) => {
                if (resdata['error']) {
                    console.error(resdata);
                } else {
    ...
    return (
        <div>
            ...
            >
                ...
                />
                <p>
                    {t('account:rewards.no-ads.p2')}{' '}
                    <strong>
                        {hideAds ? t('common:hidden') : t('common:visible')}
                    </strong>
                    .
                </p>
            </div>
            ...
            >
                ...
                <p>
                    <strong>
                        {anon
                            ? t('common:hidden').toLowerCase()
                            : t('common:visible').toLowerCase()}
                    </strong>
                    <Trans
                        i18nKey="account:rewards.credit.p2"
                        t={t}
                        values={{ hidden: '' }}
                    />
                </p>
                ...
                !patron.rewards.includes('Sponsor') && ( // Don't need to edit name if it's not shown or can be edited further above.
                    <div className={styles.row}>
                        ...
                        <Disabled
                            ...
                            tooltip={t('account:rewards.credit.invalid')}
                        >
                            <div
