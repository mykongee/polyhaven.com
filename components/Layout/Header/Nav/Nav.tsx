...
const Nav = () => {
    ...
    const [suggestLocale, setSuggestLocale] = useStoredState(
        'suggestLocale',
        true
    );
    ...
    useEffect(() => {
        const fetchHeaders = async () => {
            await fetch(`/api/reqHeaders`, { method: 'POST' })
                .then((res) => res.json())
                .then((resdata) => {
                    const reqLocales = resdata['accept-language']
                        .split(',')
                        .map((l) => l.split(';')[0]);
                    for (const locale of reqLocales) {
                        if (locales[locale]) {
                            ...
                            if (router.locale !== locale) {
                                console.log(
                                    `Suggesting locale "${locale}" based on request headers`
                                );
        ...
        <>
            <div
                className={`${styles.nav} ${
                    navHide ? styles.hiddenMobile : null
                }`}
                onClick={() => {
                    setToggle(true);
                }}
            >
                ...
                <NavItem
                    text={
                        <>
                            <Heart color="#F96854" />{' '}
                            {t('common:nav.support-us')}
                        </>
                    }
                    link="https://www.patreon.com/polyhaven/overview"
                />
                <NavItem
                    text={t('common:nav.about-contact')}
                    link="/about-contact"
                >
                    <NavItem text={t('common:nav.license')} link="/license" />
                    <NavItem
                        text={t('common:nav.news')}
                        link="https://www.patreon.com/polyhaven/posts?public=true"
                    />
                    <NavItem
                        text={t('common:nav.blog')}
                        link="https://blog.polyhaven.com"
                    />
                ...
                {user ? (
                    ...
                    <NavItem
                        text={<IoMdLogIn />}
                        link={`/account?returnTo=${router.asPath}`}
                    />
                )}
            </div>
            ...
            <div style={{ height: '100%', display: 'flex' }}>
                {suggestedLocale && suggestLocale ? (
                    <div className={styles.suggestedLocale}>
                        ...
                        />
                        <MdClose
                            title="Don't suggest a locale again"
                            onClick={() => setSuggestLocale(false)}
                        />
                    </div>
                ) : null}
                <NavItem
                    ...
                    text={
                        <LocaleFlag
                            ...
                            flag={locales[router.locale].flag}
                        />
                    }
                >
