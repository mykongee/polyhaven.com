...
const GridItem = ({
    ...
    const createTurnaround = (e) => {
        if (
            turnaroundRef &&
            turnaroundRef.current &&
            turnaroundWrapperRef &&
            turnaroundWrapperRef.current
        ) {
            setTurnaround(true);
            setHasTurned(false);
            ...
            turnaroundRef.current.src = turnaroundSrc;
            turnaroundWrapperRef.current.style.width =
                imgRef.current.offsetWidth + 'px';
            turnaroundWrapperRef.current.style.height =
                imgRef.current.offsetHeight + 'px';
            turnaroundWrapperRef.current.style.left = `${
                imgRef.current.getBoundingClientRect().left -
                wrapperRef.current.getBoundingClientRect().left
            }px`;
            turnaroundWrapperRef.current.style.top = `${
                imgRef.current.getBoundingClientRect().top -
                wrapperRef.current.getBoundingClientRect().top
            }px`;
            turnaroundRef.current.style.width =
                imgRef.current.offsetWidth * 15 + 'px'; // Force correct width, sometimes there are rounding errors
        }
    };
    const destroyTurnaround = (e) => {
        ...
        if (turnaroundRef.current.style.opacity == 1 && hasTurned) {
            turnaroundRef.current.animate([{ opacity: 1 }, { opacity: 0 }], {
                duration: 250,
            });
        }
    ...
    const updateTurnaround = (e) => {
        if (turnaround) {
            const relative =
                (e.pageX - offsetStart) / -wrapperRef.current.offsetWidth +
                1 / 15 / 4;
            ...
            >
                <div className={styles.author}>
                    <div className={styles.authorInner}>
                        {Object.keys(asset.authors)
                            .sort()
                            .map((a) => (
                                <SimpleAuthorCredit
                                    key={a}
                                    id={a}
                                    donated={asset.donated}
                                />
                            ))}
                    </div>
                </div>
                ...
                {badge ? (
                    <div
                        className={`${styles.badge} ${badge.style}`}
                        title={badge.tooltip}
                    >
                        {badge.text}
                    </div>
