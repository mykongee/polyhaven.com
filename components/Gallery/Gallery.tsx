                ...
                <div className={styles.lightboxWrapper} onClick={closeLightbox}>
                    <div className={styles.lightbox}>
                        <div className={styles.imageWrapper}>
                            ...
                            <img
                                src={`https://cdn.polyhaven.com/gallery/${lightboxData.file_name}?width=${width}`}
                            />
                        </div>
                        <p>
                            {lightboxData.artwork_name && (
                                <em>{lightboxData.artwork_name}</em>
                            )}
                            {t('by')}
                            {lightboxData.author_link &&
                            isURL(lightboxData.author_link, {
                                require_protocol: true,
                            }) ? (
                                <a
                                    target="_blank"
                                    rel="noopener"
                                    href={lightboxData.author_link}
                                >
                                    {lightboxData.author}
                                </a>
                            ) : (
                            ...
                            {lightboxData.assets_used.length > 3 ? (
                                <span
                                    title={lightboxData.assets_used.join(', ')}
                                >
                                    ...
                                </span>
                            ) : null}
