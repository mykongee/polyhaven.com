...
const EvDiff = () => {
    ...
    const [message, setMessage] = useState(
        'Fill in the fields above, the result will be shown here.'
    );
    ...
    const calcEvs = () => {
        ...
        if (dr_total >= 0) {
            return (
                'Image A is <strong>' +
                Math.abs(dr_total) +
                'EVs darker</strong> than Image B.'
            );
        } else {
            return (
                'Image A is <strong>' +
                Math.abs(dr_total) +
                'EVs brighter</strong> than Image B.'
            );
        }
    };
    ...
    return (
        <div className={styles.wrapper}>
            ...
            <p>
                Use this page to calculate the exposure difference in stops/EVs
                between two images by inputting the shutter speed, aperture, ISO
                and ND filter reduction (if any).
            </p>
            ...
            <form onSubmit={(e) => e.preventDefault()}>
                <div className={styles.formItem}>
                    ...
                    <input
                        type="text"
                        value={apertureA}
                        onChange={(e) => set_apertureA(e.target.value)}
                        placeholder="Aperture"
                    />
                    <input
                        type="text"
                        value={isoA}
                        onChange={(e) => set_isoA(e.target.value)}
                        placeholder="ISO"
                    />
                    <input
                        type="text"
                ...
                <div className={styles.formItem}>
                    ...
                    <input
                        type="text"
                        value={apertureB}
                        onChange={(e) => set_apertureB(e.target.value)}
                        placeholder="Aperture"
                    />
                    <input
                        type="text"
                        value={isoB}
                        onChange={(e) => set_isoB(e.target.value)}
                        placeholder="ISO"
                    />
                    <input
                        type="text"
                ...
                <div className={styles.formItem}>
                    {message === 'Complete' ? (
                        result >= 0 ? (
                            <h2>
                                Image A is <pre>{Math.abs(result)}</pre> EVs
                                darker than Image B.
                            </h2>
                        ) : (
                            <h2>
                                Image A is <pre>{Math.abs(result)}</pre> EVs
                                brighter than Image B.
                            </h2>
                        )
                    ) : (
                        <h2>{message}</h2>
            ...
            <ul className={styles.info}>
                <li>
                    <b>Shutter Speed</b> is in seconds, and supports fractions.
                    E.g. <pre>1/4000</pre> for 1/4000th of a second, or{' '}
                    <pre>30</pre> for 30 seconds.
                </li>
                <li>
                    <b>Aperture</b> should be without the "F/". E.g.{' '}
                    <pre>8</pre> for F/8, or <pre>2.2</pre> for F/2.2.
                </li>
                <li>
                    <b>ISO</b> is as you'd expect. If left blank, will default
                    to <pre>100</pre>.
                </li>
                <li>
                    In the <b>Filter</b> box, type the number of EVs/stops that
                    your filter darkens an image. E.g. for a 3-stop filter (AKA
                    "ND8") type <pre>3</pre>, or a 12-stop filter (AKA "ND4096")
                    type <pre>12</pre>. If no filter was used, leave it blank or
                    type <pre>0</pre>.
                </li>
            </ul>
        </div>
