...
export async function getStaticProps(context) {
    ...
    const aMonthAgo = new Date(Date.now() - 30 * msPerDay)
        .toISOString()
        .split('T')[0];
    const threeMonthsAgo = new Date(Date.now() - (91 + 6) * msPerDay)
        .toISOString()
        .split('T')[0];
    ...
    // Relative type graph
    const relativeType = await fetch(
    ...
    for (const info of Object.values(assets)) {
        const month = new Date(info['date_published'] * 1000)
            .toISOString()
            .substring(0, 7);
        if (month > today) {
            continue;
    ...
    for (const stat of resolutionsData) {
        const type =
            Object.keys(resolutions)[parseInt(stat.slug.substring(1, 2))];
        if (!type) continue;
    ...
    for (const [type, reses] of Object.entries(resolutions)) {
        const sortedKeys = Object.keys(reses).sort(
    ...
    for (const stat of formatsData) {
        ...
        if (
            type === 'hdris' &&
            ['jpg_plain', 'jpg_pretty', 'raw'].includes(fmtStr)
        ) {
            fmtStr = 'backplate';
        }
    ...
    for (const [type, fmts] of Object.entries(formats)) {
        const sortedKeys = Object.keys(fmts).sort(
            (a, b) => parseInt(a) - parseInt(b)
        );
        let tmp = {};
        for (const r of sortedKeys) {
    ...
    return {
        props: {
            ...
            datasets: {
                threeMonths: {
                    hdris: threeMonthsHDRI,
                    textures: threeMonthsTex,
                    models: threeMonthsMod,
                },
                relativeType,
                relativecategory,
                monthlyAssets,
