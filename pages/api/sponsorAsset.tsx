...
const Route = async (req, res) => {
    ...
    data.key = crypto
        .createHmac('sha256', process.env.PATRON_INFO_KEY)
        .update(data.uuid)
        .digest('hex');
        ...
        if (!v) {
            ...
            if (v === assetID) {
                res.status(400).json({
                    error: '400',
                    message:
                        "You're already sponsoring this asset! Please wait a few hours for this page to update.",
                });
                return;
            }
        }
    ...
    if (!returnData.error) {
        returnData = {
            error: null,
            message: 'Done! This page may take a few hours to update.',
        };
    }
